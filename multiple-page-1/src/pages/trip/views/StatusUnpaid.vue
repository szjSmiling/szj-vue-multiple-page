<template>
  <div>
      ...
      <div class="confirm-info-detail-box2">
        <div v-for="(item, key) in trip.triplist" :key="key">
          ...
          <template v-for="(travel, index)  in item.travellerinfo" >
            ...
            :ticketInfo="item.ticketsinfo[0][index]" :tripTraveller="travel" :key="travel.name" />
          </template>
        </div>
      </div>
    ...
    <div class="need-pay bgColorGray">
      <p class="sign-pay-text">Finish payment in 30 minutes in case price change. If you have paid, Please ignore this message.</p>
      <a class="need-pay-btn" :href="`/payment/${orderId}/${orderInfo.cancatInfo}`">Finish Payment</a>
    </div>
  </div>
</template>
<script>
